...
Run `npm run check` before handing off a change. Unit tests use Node's test runner
through `tsx` and live under `test/identity/`.
...
This repository currently owns only the identity/profile foundation:
...
Chain access, network fetching, invocation, receipt signing, reputation, and UI
work are outside this boundary until explicitly added with tests and public design
documentation.
